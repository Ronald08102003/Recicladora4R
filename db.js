...
// Servir archivos estáticos (CSS, JS, imágenes) desde la raíz
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT id, nombre, usuario, clave, rol FROM usuarios WHERE usuario = $1 LIMIT 1',
      [usuario]
    );
    ...
    console.log(`✅ Usuario ${usuario} logueado correctamente`);
...
app.get('/test-db', async (req, res) => {
  try {
    const result = await pool.query('SELECT id, usuario, clave, rol FROM usuarios LIMIT 5');
    res.json(result.rows);
  } catch (err) {
