...
const pool = require('./db'); // Conexión configurada en db.js
...
// Servir archivos estáticos desde la raíz (CSS, Imágenes, JS frontal)
...
// ================= RUTAS HTML (CORRECCIÓN DE ERRORES DE RUTA) =================
// Usamos path.join(__dirname) para que Render localice los archivos físicos correctamente
...
// Rutas de paneles y gestión administrativa
...
// ================= API LOGIN (CONTROL DE ACCESO POR ROLES) =================
app.post('/api/login', async (req, res) => {
    try {
        ...
        // trim() asegura que no fallen las claves por espacios invisibles en la BD
        if (clave.trim() !== user.clave.trim()) {
            return res.json({ success: false, message: 'Clave incorrecta' });
        }

        // Se envía la ruta lógica definida arriba para la redirección
        res.json({
            success: true,
            userId: user.id,
...
app.post('/api/finalizar-pedido', async (req, res) => {
    ...
    try {
        await pool.query('BEGIN'); // Transacción segura para integridad de datos
